...
if __name__ == "__main__":
    ...
    feats = np.array(df["features"].tolist())
    feats_delta=np.array(df_delta["features"].tolist())
    feats_delta_labels=np.array(df_delta["labels"].tolist())
    feats_dd=np.array(df_dd["features"].tolist())
    feats_dd_labels=np.array(df_dd["labels"].tolist())
    unique_labels=[]
    for ite in labels:
    ...
    for l in unique_labels:
        if(l==""):
            l="space"
        sep[l]=pd.DataFrame()
        sep_d[l]=pd.DataFrame()
        sep_dd[l]=pd.DataFrame()
    for i in range(0,number_of_examples):
        if(labels[i]==""):
            labels[i]="space"
        if(feats_delta_labels[i]==""):
            feats_delta_labels[i]="space"
        if(feats_dd_labels[i]==""):
            feats_dd_labels[i]="space"
        s1=pd.Series(feats[i])
        sep[labels[i]]=sep[labels[i]].append(s1,ignore_index=True)
        s2=pd.Series(feats_delta[i])
        sep_d[feats_delta_labels[i]] = sep_d[feats_delta_labels[i]].append(s2,ignore_index=True)
        s3=pd.Series(feats_dd[i])
        sep_dd[feats_dd_labels[i]]=sep_dd[feats_dd_labels[i]].append(s3,ignore_index=True)

    for k in sep:
        fname=str(k)+"_train.hdf"
        sep[k].to_hdf("./features_seperated/mfcc/"+fname, k)
        fname=str(k)+"_train.hdf"
        sep_d[k].to_hdf("./features_seperated/mfcc_delta/"+fname,k)
        fname=str(k)+"_train.hdf"
        sep_dd[k].to_hdf("./features_seperated/mfcc_delta_delta/"+fname,k)

    lns=open("phone.txt",'w')
    for l in unique_labels:
        if(l==""):
            lns.write("space\n")
        else:
